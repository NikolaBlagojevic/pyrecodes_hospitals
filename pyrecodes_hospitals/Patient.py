...
class PatientType():
    ...
    def get_resource_demand(self) -> dict:
        if not(self.out_of_hospital()):
            self.update_stretcher_demand()
            return self.demand[self.get_current_department_id()]
        else:
            return {}
    
    def update_stretcher_demand(self) -> None:
        ...
        if self.get_current_length_of_stay() > 0:
            self.demand[self.get_current_department_id()][self.STRETCHER_NAME] = 0.0
    ...
    def update_patient_status_when_demand_not_met(self, resource_name: str, demand_met: float) -> None:
        ...
        for consequence in consequences:
            ...
            if consequence_name == 'Mortality Rate Increase [per missing nurse]':
                self.mortality_rate = self.update_patient_when_nurses_missing(resource_name, consequence_value, demand_met, self.mortality_rates[self.get_current_department_id()])
            elif consequence_name == 'Length Of Stay Extended [per missing nurse]':
                self.length_of_stay = self.update_patient_when_nurses_missing(resource_name, consequence_value, demand_met, self.lengths_of_stay[self.get_current_department_id()])            
            elif consequence_name == 'Mortality Rate Increase':
                self.mortality_rate = consequence_value * self.mortality_rates[self.get_current_department_id()]
                self.patient_not_treated()
            elif consequence_name == 'Length Of Stay Extended':
                self.length_of_stay = consequence_value * self.lengths_of_stay[self.get_current_department_id()]
                self.patient_not_treated()
    ...
    def update_patient_when_nurses_missing(self, resource_name: str, consequence_value: float, demand_met: float, parameter_to_update: float) -> None:
        self.patient_not_treated()
        return parameter_to_update * max(consequence_value ** (self.demand[self.get_current_department_id()][resource_name] * (1-demand_met)), 1.0)
